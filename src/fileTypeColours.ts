...
const FILE_TYPE_EXTENSIONS: Record<FileTypeCategory, string[]> = {
  media: [
    'png', 'jpg', 'jpeg', 'gif', 'svg', 'webp', 'bmp', 'tif', 'tiff', 'heic', 'heif', 'ico',
    'mp3', 'wav', 'flac', 'aac', 'ogg', 'm4a', 'aiff', 'wma',
    'mp4', 'mkv', 'avi', 'mov', 'webm', 'wmv', 'm4v', 'mpeg', 'mpg',
  ],
  documents: [
    'pdf', 'doc', 'docx', 'txt', 'rtf', 'md', 'markdown', 'odt', 'xls', 'xlsx', 'csv', 'tsv',
    'ppt', 'pptx', 'key', 'pages', 'numbers', 'epub',
  ],
  code: [
    'js', 'jsx', 'ts', 'tsx', 'py', 'go', 'rs', 'java', 'c', 'cpp', 'h', 'hpp', 'cs', 'php',
    'rb', 'swift', 'kt', 'scala', 'sh', 'bash', 'zsh', 'fish', 'ps1', 'html', 'css', 'scss',
    'sass', 'less', 'json', 'yml', 'yaml', 'toml', 'xml', 'sql', 'graphql', 'gql', 'vue', 'svelte',
  ],
  archives: [
    'zip', 'tar', 'tar.gz', 'tar.bz2', 'tar.xz', 'tar.zst', 'gz', 'tgz', 'bz2', 'tbz2', 'xz',
    '7z', 'rar', 'zst',
  ],
  system: [
    'log', 'conf', 'config', 'ini', 'cfg', 'env', 'bak', 'tmp', 'temp', 'lock', 'pid', 'service',
  ],
};

const extensionToCategory = new Map<string, FileTypeCategory>();
for (const [category, extensions] of Object.entries(FILE_TYPE_EXTENSIONS) as Array<[FileTypeCategory, string[]]>) {
...
export const getFileTypeCategory = (fileName: string, isDirectory: boolean): FileTypeCategory | null => {
